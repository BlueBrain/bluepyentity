...
from entity_management.nexus import factory as test_module
...
def test_entity_factory_open_instance():
    ...
    with patch(
        "entity_management.nexus.tools.open_circuit_snap", return_value=mocked_instance
    ) as mocked_opener:
        factory = test_module.EntityFactory(helper=MagicMock(), connector=MagicMock())
...
def test_entity_factory_open_instance_with_default_tool():
    ...
    with patch(
        "entity_management.nexus.tools.open_emodelconfiguration", return_value=mocked_instance
    ) as mocked_opener:
        connector = MagicMock()
...
def test_entity_factory_fail_to_open_instance():
    with patch("entity_management.nexus.tools.open_circuit_snap", side_effect=Exception("error")):
        factory = test_module.EntityFactory(helper=MagicMock(), connector=MagicMock())
        resource = Resource(type="DetailedCircuit")
